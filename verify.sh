...
if [ -n "$NEW_L" ]; then
  ...
  fail "Cluster could not elect leader after split election (2 of $TOTAL_NODES dead)"
  info "  EXPECTED for 3-node cluster: with 2 dead, quorum (2/3) lost — no leader is CORRECT"
fi
