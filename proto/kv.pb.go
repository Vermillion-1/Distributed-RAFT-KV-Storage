...
type GetRequest struct {
	...
	FollowerRead  bool                   `protobuf:"varint,2,opt,name=follower_read,json=followerRead,proto3" json:"follower_read,omitempty"` // If true, follower serves read via read-index (linearizable, distributes load)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *GetRequest) GetFollowerRead() bool {
	if x != nil {
		return x.FollowerRead
	}
	return false
}
...
type GetReadIndexResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	CommitIndex   uint64                 `protobuf:"varint,1,opt,name=commit_index,json=commitIndex,proto3" json:"commit_index,omitempty"` // Leader's current commit index; follower waits until applied_index >= this
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *GetReadIndexResponse) GetCommitIndex() uint64 {
	if x != nil {
...
const file_proto_kv_proto_rawDesc = "" +
	...
	"\rfollower_read\x18\x02 \x01(\bR\ffollowerRead\"Z\n" +
	...
	"\x04term\x18\x06 \x01(\x04R\x04term\"\x15\n" +
	"\x13GetReadIndexRequest\"9\n" +
	"\x14GetReadIndexResponse\x12!\n" +
	"\fcommit_index\x18\x01 \x01(\x04R\vcommitIndex2\xcd\x02\n" +
	"\aKVStore\x12,\n" +
	...
	"\x06Health\x12\x14.store.HealthRequest\x1a\x15.store.HealthResponse\x12G\n" +
	"\fGetReadIndex\x12\x1a.store.GetReadIndexRequest\x1a\x1b.store.GetReadIndexResponseB!Z\x1fgithub.com/ankush/raft-kv/protob\x06proto3"
...
var file_proto_kv_proto_goTypes = []any{
	...
	(*GetReadIndexRequest)(nil),  // 10: store.GetReadIndexRequest
	(*GetReadIndexResponse)(nil), // 11: store.GetReadIndexResponse
}
var file_proto_kv_proto_depIdxs = []int32{
	...
	10, // 5: store.KVStore.GetReadIndex:input_type -> store.GetReadIndexRequest
	3,  // 6: store.KVStore.Get:output_type -> store.GetResponse
	5,  // 7: store.KVStore.Set:output_type -> store.SetResponse
	7,  // 8: store.KVStore.Delete:output_type -> store.DeleteResponse
	1,  // 9: store.KVStore.Join:output_type -> store.JoinResponse
	9,  // 10: store.KVStore.Health:output_type -> store.HealthResponse
	11, // 11: store.KVStore.GetReadIndex:output_type -> store.GetReadIndexResponse
