...
func main() {
	...
	if *join == "" {
		...
		for i := 0; i < retries; i++ {
			...
			// A3 (GCP_TODO.md): exponential backoff to avoid thundering herd on new leader
			backoff := time.Duration(1<<uint(i)) * time.Second
			if backoff > 30*time.Second {
				backoff = 30 * time.Second
			}
			log.Printf("Failed to join cluster (attempt %d/%d): %v. Retrying in %s...", i+1, retries, err, backoff)
			time.Sleep(backoff)
		}
...
func joinCluster(targetGrpc, nodeID, raftAddr, grpcAddr string) error {
	...
	// A2 (GCP_TODO.md): 5s deadline prevents hung goroutine if target VM is unreachable
