#!/bin/bash
# ═══════════════════════════════════════════════════════════════════
# verify_phase4.sh — Phase 4: Durability Tests
# Usage: bash verify_phase4.sh [port]   e.g. bash verify_phase4.sh 8080
#
...
#
# Theory: All committed writes are stored in BoltDB on disk.
# Restarting all nodes should restore the full state by replaying
# the Raft log from disk.
# ═══════════════════════════════════════════════════════════════════
header "D1: Total Wipe -- Full Cluster Restart"

L_PORT=$(grpc_port "$CUR_L")
info "Writing 10 known keys through leader (:${L_PORT})..."

for i in $(seq 1 10); do
  "${KV_CLIENT}" -addr "127.0.0.1:${L_PORT}" -cmd set \
    -key "d1_key_${i}" -val "d1_val_${i}" 2>/dev/null
done
sleep 2

# Verify writes landed before killing
PRE_CHECK=0
for i in $(seq 1 10); do
  result=$("${KV_CLIENT}" -addr "127.0.0.1:${L_PORT}" -cmd get -key "d1_key_${i}" 2>&1)
...
if [ -z "$NEW_L" ]; then
  ...
  sleep 5
  NEW_L_PORT=$(grpc_port "$NEW_L")

  recovered=0
  missing_keys=""
  for i in $(seq 1 10); do
    result=$("${KV_CLIENT}" -addr "127.0.0.1:${NEW_L_PORT}" -cmd get \
      -key "d1_key_${i}" 2>&1)
...
#
# Theory: A write is only acknowledged after it's committed to quorum.
# If the leader dies mid-write, the write either committed (and other
# nodes have it) or didn't commit (and the client got an error).
# Either way: no corruption, no partial writes.
# ═══════════════════════════════════════════════════════════════════
...
if [ -n "$CUR_L" ]; then
  ...
  (
    for i in $(seq 1 50); do
      result=$("${KV_CLIENT}" -addr "127.0.0.1:${L_PORT}" -cmd set \
  ...
  # Wait a bit then kill the leader mid-write
  sleep 0.5
  info "Killing leader ${CUR_L} mid-write..."
  kill_node "$CUR_L"

  # Wait for the writer to finish (it will get errors once leader dies)
  ...
  if [ -n "$NEW_L" ]; then
    ...
    # Verify all acknowledged writes are present
    lost=0
  ...
  # Restart killed node for next test
...
# ═══════════════════════════════════════════════════════════════════
# D3 — SNAPSHOT RECOVERY: Does a restarted node catch up?
#
# Theory: Raft periodically compacts its log into a snapshot. On
# restart, a node loads the snapshot + replays any remaining log
# entries to fully catch up.
#
# Note: The default snapshot threshold (~8192 entries) may be too
# high for a short test. We test that log replay works correctly
# and note snapshot-based recovery as a known limitation of this
# test duration.
# ═══════════════════════════════════════════════════════════════════
...
" 2>/dev/null)
  ...
  # Write entries while the follower is dead
  ...
  # Verify the restarted node has the data we wrote
  F_PORT=$(grpc_port "$FOLLOWER")
  # Read through the leader (follower redirects to leader)
  readable=0
  for i in $(seq 1 30); do
    result=$("${KV_CLIENT}" -addr "127.0.0.1:${F_PORT}" -cmd get \
      -key "d3_key_${i}" 2>&1)
